    ...
    <div class="app-container calendar-list-container">
        ...
        <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible" @close="resetTemp">
            <el-form :rules="rules" ref="dataForm" :model="temp" label-position="left" label-width="180px" style='width: 500px; margin-left:50px;'>
                ...
                <el-form-item label="商户名称" prop="user_name" v-show="dialogStatus=='create'">
                    <el-select v-model="temp.user_name" filterable placeholder="请选择" :loading="tempUserIdLoading" @change="handleChange">
                        <el-option
                            v-for="item in tempUserOption"
                            :key="item.value"
                            :label="item.label"
                            :value="item.value">
                        </el-option>
                ...
                <el-form-item label="商户名称" prop="user_name"  v-show="dialogStatus=='update'">
                    <el-input type="text" v-model="temp.user_name" disabled>
                    </el-input>
                </el-form-item>
...
<script>
import { fetchList, userNameSelect, selectUserNo, createReportSend, updateReportSend, deleteReportSend } from '@/admin/api/reportSendSetting'
...
export default {
    ...
    data () {
        return {
            ...
            rules: {
                ...
                user_name: [{ required: true, message: '请选择用户', trigger: 'change' }],
    ...
    methods: {
        ...
        selectUserName () {
            if (this.tempUserOption.length === 0) {
                this.tempUserIdLoading = true

                userNameSelect().then((response) => {
                    this.tempUserIdLoading = false

                    if (response.data.Status === '200') {
                        this.tempUserOption = response.data.Content
                    } else {
                        this.$notify({
                            title: '失败',
