...
<script>
import { fetchList, updatewithdrawalReportStatus, fetchCapital } from '@/admin/api/withdrawalReport'
...
export default {
    ...
    methods: {
        ...
        handleDownload () {
            ...
            const tHeader = ['状态', '用户名', '商户号', '金额(RMB)', '余额(RMB)', '通道', '订单号', '手续费(RMB)', '银行', '银行账号', '收款人', '申请时间']
            const filterVal = ['status', 'user_name', 'merchant_no', 'apply_amount', 'now_balance', 'pay_way_name', 'merchant_order_no', 'payer_fee', 'pay_type_name', 'pay_type_code', 'receiver_name', 'create_time']
            const data = this.formatJson(filterVal, this.list)

            exportJsonToExcel(tHeader, data, '出金报表')
