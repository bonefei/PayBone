...
class IpsPaySubmit
{
    ...
    /**
     * 建立请求，以表单HTML形式构造（默认）--商城中专
     * @param $para_temp 请求前的参数数组
     * @return 提交表单HTML文本
     */
    public function buildRequestFormRedirect($para_temp){
        //待请求参数xml
        $para = $this->buildRequestPara($para_temp);

        $sHtml = "<form id='ipspaysubmit' name='ipspaysubmit' method='post' action='" . $this->ipspay_config['RedirectUrl'] . "'>";
        $sHtml .= "<input type='hidden' name='action' value='" . $this->ipspay_config['PostUrl']. "'/>";
        $sHtml .= "<input type='hidden' name='para_temp' value='" . base64_encode($para) . "'/>";
    ...
    public function buildHead($para_temp)
    {
        ...
        $sReqXmlHead .= "<Signature>" . IpsFun::md5Sign($this->buildBody($para_temp), $para_temp["MerCode"], $this->ipspay_config['MerCert']) . "</Signature>";
        $sReqXmlHead .= "</head>";
        return $sReqXmlHead;
    }
    /**
