...
class SystemController extends Controller
{
     ...
     * 获取网关黑名单商户号
     */
    public function gatewayBlacklistSelectPayCode(Request $request)
    {
        ...
        if ($validator->fails()) {
            return response()->json([ 'Status' => 'GBSP100', 'Content' => '请按照提示将表单填写完整' ], self::$successStatus);
        }
    ...
    public function gatewayBlacklistCreate(Request $request)
    {
        ...
        if ($validator->fails()) {
            return response()->json([ 'Status' => 'GBC100', 'Content' => '请按照提示将表单填写完整' ], self::$successStatus);
        }
     ...
     * 获取网关白名单商户号
     */
    public function gatewayWhitelistSelectPayCode(Request $request)
    {
        $validator = Validator::make($request->all(), [
            'user_name' => 'required'
        ]);
